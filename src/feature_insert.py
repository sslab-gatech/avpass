...
# prepare this before you run this script
...
def unpack_apk(apkname):
    ...
    if not os.path.exists(apkdir):
        ...
        os.system("java -jar apktool.jar d " + apkname)
    else:
...
def pack_apk(dirname, out_filename):
    ...
    os.system("java -jar apktool.jar b {} -o {}".format(dirname, out_filename))
    ...
    for root, dirs, files in os.walk(out + '/smali'):
        for file_ in files:
            if file_ == 'MainActivity.smali':
                ...
                break
    if not target:
        # TODO what is the target if we ignore the path?
        print '[*] cannot find MainActivity.smali'
        sys.exit(0)
    # TODO where do you want to put the code?
    #return target
    return '{}/smali/com/coffee/coffeecup/MainActivity.smali'.format(out)
...
def insert_string(source, out, flag):
    ...
    for line in f_source:
        ...
        # v{number} to v0 or v{number} to v{1, 2, ...}
        # TODO: check both
        # dont need to do this: already checked
...
def insert_variable(source, out, flag):
    ...
    for line in f_source:
        ...
        # v{number} to v0 or v{number} to v{1, 2, ...}
        # TODO: check if this works without modify v{number}
        line = re.sub(r' p[0-9]{1,2}', ' p1', line)
        out_add.append(line)
...
def insert_resource(source, out, flag):
    ...
    for root, dirs, files in os.walk(source + '/res'):
        for file_ in files:
            #if file_.endswith('.png') or file_ == 'strings.xml':
            if True:
                words = root.split('/')
...
def do_add(source_dir, out_dir, flag):
    os.system('cp -r empty temp_{}'.format(source_dir))
    ...
    #unpack_apk(source_dir + '_' + '_'.join(args) + '.apk')
    #os.system('mv temp {}'.format(source_dir + '_' + '_'.join(args) + '_'))
...
if __name__ == '__main__':
    ...
    #unpack_apk(out_filename)
    #out_dir = getdir(out_filename)
    ...
    do_add(source_dir, out_dir, flag)
